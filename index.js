...
async function main() {
    await fetch(`https://discord.com/api/v9/channels/${config.channel}/attachments`, {
        method: "POST",
        headers: {
            "Host": "discord.com",
        ...
        res.text().then(async (body) => {
            ...
            link = data.attachments[0].upload_url.split("/")[3] + "/" + data.attachments[0].upload_url.split("/")[4].split("?")[0];
            await fetch(data.attachments[0].upload_url, {
                ...
                if (res.status !== 200) return console.log(res.status);
                await fetch(`https://discord.com/api/v9/channels/${config.channel}/messages`, {
                    method: "POST",
                    headers: {
                        "Host": "discord.com",
                        "Authorization": config.token,
